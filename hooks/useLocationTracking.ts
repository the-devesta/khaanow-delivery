...
export function useLocationTracking(options: LocationTrackingOptions = {}) {
  ...
  const updateBackendLocation = useCallback(async (latitude: number, longitude: number) => {
    try {
  ...
  const startTracking = useCallback(async () => {
    ...
    if (initialLocation) {
      await updateBackendLocation(initialLocation.latitude, initialLocation.longitude);
    }
  ...
  }, [requestPermissions, getCurrentLocation, updateBackendLocation, distanceThreshold, updateInterval, isOnline]);
